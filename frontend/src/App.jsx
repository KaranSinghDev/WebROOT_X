        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 font-mono">
              Client-Side HEP Analysis / C++17 / WebAssembly
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 bg-gray-800 rounded-full border border-gray-700">
          <div
            className={`w-2 h-2 rounded-full ${
              wasmModule ? "bg-green-500 animate-pulse" : "bg-yellow-500"
            }`}
          ></div>
        ...
        <div className="bg-slate-800 p-6 rounded-xl border border-gray-700 shadow-lg">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">
              Kinematics Input (GeV)
            </h2>
            <button
              onClick={randomize}
              className="text-xs bg-slate-700 hover:bg-slate-600 px-2 py-1 rounded flex items-center gap-1 transition"
            >
          ...
          <div className="space-y-6">
            {/* Particle 1 */}
            <div>
              <label className="text-xs text-blue-400 font-bold mb-2 block">
                PARTICLE 1 (Electron)
              </label>
                ...
                {Object.keys(p1).map((key) => (
                  <div key={`p1-${key}`}>
                    ...
                    <input
                      ...
                      value={p1[key]}
                      onChange={(e) =>
                        handleInputChange(1, key, e.target.value)
                      }
                    />
                  </div>
                ))}
            ...
            {/* Particle 2 */}
            <div>
              <label className="text-xs text-red-400 font-bold mb-2 block">
                PARTICLE 2 (Positron)
              </label>
              <div className="grid grid-cols-2 gap-3">
                {Object.keys(p2).map((key) => (
                  <div key={`p2-${key}`}>
                    ...
                    <input
                      ...
                      value={p2[key]}
                      onChange={(e) =>
                        handleInputChange(2, key, e.target.value)
                      }
                    />
                  </div>
                ))}
